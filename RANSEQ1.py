...
def ranGen(inputseq):
#Define a function to generate random sequence which based on the frequency of every nucleotide in the input file and print the sequence on the screen and write it in the output file.  
  fa, fc, fg, ft = calFrequency(inputseq)
  ranSeq = []
  nucleotide = ['A', 'C', 'G', 'T']
  ranSeq = np.random.choice(nucleotide, len(inputseq), p=[fa, fc, fg, ft])
  newSeq = ''
  newSeq = "".join(str(x) for x in ranSeq)  
  print newSeq
  f.write("\n")
  f.write(newSeq)
  return newSeq
...
#Main function which execute the command line and call the functions above
  ...
  global f
  f= open(outputfile, 'w')
  f.write("Random sequence generation:")
  calMeanAndSD(int(n))		 
